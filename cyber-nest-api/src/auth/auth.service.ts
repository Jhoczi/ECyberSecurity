...
export class AuthService {
    ...
    async validateUserData(oldPassword: string, newPassword: string, repeatNewPassword: string, userEmail: string) {
        const passwordSettings = await this.prisma.passwordSettings.findFirst({});

        if (newPassword !== repeatNewPassword) {
            throw new ForbiddenException('New Password and Repeat New Password do not match');
        }
        ...
        if (newPassword.length !== passwordSettings.passwordLength) {
            throw new ForbiddenException(`Expected password length should be ${passwordSettings.passwordLength}`);
        }
    ...
    async updateUserPassword(userDto: UserRequest): Promise<UserResponse> {
        await this.validateUserData(userDto.oldPassword, userDto.newPassword, userDto.repeatNewPassword, userDto.email);
    ...
    async getPasswordSettings(userEmail: string): Promise<Pwd> {
        ...
        let expireDate: Date = new Date(user.pwdDateUpdate.getTime() + (pwdSettings.passwordDaysDuration * 24 * 60 * 60 * 1000));
    ...
    async setPasswordSettings(passwordDto: Pwd): Promise<Pwd> {
        ...
        let expireDate: Date = new Date(user.pwdDateUpdate.getTime() + (settings.passwordDaysDuration * 24 * 60 * 60 * 1000));
        const result: Pwd = {
            expireAt: expireDate,
